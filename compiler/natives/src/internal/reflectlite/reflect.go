...
func reflectType(typ *js.Object) *rtype {
	if typ.Get("reflectliteType") == js.Undefined {
		rt := &rtype{
		...
		typ.Set("reflectliteType", js.InternalObject(rt))
	...
	return (*rtype)(unsafe.Pointer(typ.Get("reflectliteType").Unsafe()))
}
...
func MakeSlice(typ Type, len, cap int) Value {
	if typ.Kind() != Slice {
		panic("reflectlite.MakeSlice of non-slice type")
	}
	if len < 0 {
		panic("reflectlite.MakeSlice: negative len")
	}
	if cap < 0 {
		panic("reflectlite.MakeSlice: negative cap")
	}
	if len > cap {
		panic("reflectlite.MakeSlice: len > cap")
	}
...
func FuncOf(in, out []Type, variadic bool) Type {
	if variadic && (len(in) == 0 || in[len(in)-1].Kind() != Slice) {
		panic("reflectlite.FuncOf: last arg of variadic func must be slice")
	}
...
func MapOf(key, elem Type) Type {
	switch key.Kind() {
	case Func, Map, Slice:
		panic("reflectlite.MapOf: invalid key type " + key.String())
	}
...
func MakeFunc(typ Type, fn func(args []Value) (results []Value)) Value {
	if typ.Kind() != Func {
		panic("reflectlite: call of MakeFunc with non-Func type")
	}
...
func cvtDirect(v Value, typ Type) Value {
	...
	default:
		panic(&ValueError{"reflectlite.Convert", k})
	}
...
func Copy(dst, src Value) int {
	...
	if dk != Array && dk != Slice {
		panic(&ValueError{"reflectlite.Copy", dk})
	}
	if dk == Array {
	...
	if sk != Array && sk != Slice {
		...
		if !stringCopy {
			panic(&ValueError{"reflectlite.Copy", sk})
		}
	}
	src.mustBeExported()

	if !stringCopy {
		typesMustMatch("reflectlite.Copy", dst.typ.Elem(), src.typ.Elem())
	}
...
func valueInterface(v Value, safe bool) interface{} {
	...
	if safe && v.flag&flagRO != 0 {
		panic("reflectlite.Value.Interface: cannot return value obtained from unexported field or method")
	}
...
func makeMethodValue(op string, v Value) Value {
	if v.flag&flagMethod == 0 {
		panic("reflectlite: internal error: invalid use of makePartialFunc")
	}
...
func (t *rtype) Method(i int) (m Method) {
	...
	if i < 0 || i >= len(methods) {
		panic("reflectlite: Method index out of range")
	}
	p := methods[i]
...
func (v Value) Cap() int {
	...
	panic(&ValueError{"reflectlite.Value.Cap", k})
}
...
func (v Value) Field(i int) Value {
	if v.kind() != Struct {
		panic(&ValueError{"reflectlite.Value.Field", v.kind()})
	}
	tt := (*structType)(unsafe.Pointer(v.typ))
	if uint(i) >= uint(len(tt.fields)) {
		panic("reflectlite: Field index out of range")
	}
...
func (v Value) Index(i int) Value {
	...
	case Array:
		...
		if i < 0 || i > int(tt.len) {
			panic("reflectlite: array index out of range")
		}
		typ := tt.elem
	...
	case Slice:
		...
		if i < 0 || i >= s.Get("$length").Int() {
			panic("reflectlite: slice index out of range")
		}
	...
	case String:
		str := *(*string)(v.ptr)
		if i < 0 || i >= len(str) {
			panic("reflectlite: string index out of range")
		}
	...
	default:
		panic(&ValueError{"reflectlite.Value.Index", k})
	}
}
...
func (v Value) Len() int {
	...
	default:
		panic(&ValueError{"reflectlite.Value.Len", k})
	}
}
...
func (v Value) Pointer() uintptr {
	...
	default:
		panic(&ValueError{"reflectlite.Value.Pointer", k})
	}
}
...
func (v Value) Set(x Value) {
	v.mustBeAssignable()
	x.mustBeExported()
	x = x.assignTo("reflectlite.Set", v.typ, nil)
	if v.flag&flagIndir != 0 {
		switch v.typ.Kind() {
		case Array:
...
func (v Value) SetBytes(x []byte) {
	...
	if v.typ.Elem().Kind() != Uint8 {
		panic("reflectlite.Value.SetBytes of non-byte slice")
	}
...
func (v Value) SetCap(n int) {
	...
	if n < s.Get("$length").Int() || n > s.Get("$capacity").Int() {
		panic("reflectlite: slice capacity out of range in SetCap")
	}
...
func (v Value) SetLen(n int) {
	...
	if n < 0 || n > s.Get("$capacity").Int() {
		panic("reflectlite: slice length out of range in SetLen")
	}
	newSlice := jsType(v.typ).New(s.Get("$array"))
	newSlice.Set("$offset", s.Get("$offset"))
...
func (v Value) Slice(i, j int) Value {
	...
	case Array:
		if v.flag&flagAddr == 0 {
			panic("reflectlite.Value.Slice: slice of unaddressable array")
		}
		tt := (*arrayType)(unsafe.Pointer(v.typ))
		cap = int(tt.len)
		typ = SliceOf(tt.elem)
	...
	case String:
		...
		if i < 0 || j < i || j > len(str) {
			panic("reflectlite.Value.Slice: string slice index out of bounds")
		}
		return ValueOf(str[i:j])

	default:
		panic(&ValueError{"reflectlite.Value.Slice", kind})
	}

	if i < 0 || j < i || j > cap {
		panic("reflectlite.Value.Slice: slice index out of bounds")
	}
...
func (v Value) Slice3(i, j, k int) Value {
	...
	default:
		panic(&ValueError{"reflectlite.Value.Slice3", kind})
	}

	if i < 0 || j < i || k < j || k > cap {
		panic("reflectlite.Value.Slice3: slice index out of bounds")
	}
...
func (e *ValueError) Error() string {
	if e.Kind == 0 {
		return "reflectlite: call of " + e.Method + " on zero Value"
	}
	return "reflectlite: call of " + e.Method + " on " + e.Kind.String() + " Value"
}
...
func methodReceiver(op string, v Value, i int) (_ *rtype, t *funcType, fn unsafe.Pointer) {
	...
	if v.typ.Kind() == Interface {
		...
		if i < 0 || i >= len(tt.methods) {
			panic("reflectlite: internal error: invalid method index")
		}
		m := &tt.methods[i]
		if !tt.nameOff(m.name).isExported() {
			panic("reflectlite: " + op + " of unexported method")
		}
...
func (v Value) call(op string, in []Value) []Value {
	...
	if fn == nil {
		panic("reflectlite.Value.Call: call of nil function")
	}
	...
	if isSlice {
		if !t.IsVariadic() {
			panic("reflectlite: CallSlice of non-variadic function")
		}
		if len(in) < n {
			panic("reflectlite: CallSlice with too few input arguments")
		}
		if len(in) > n {
			panic("reflectlite: CallSlice with too many input arguments")
		}
	} else {
		if t.IsVariadic() {
			n--
		...
		if len(in) < n {
			panic("reflectlite: Call with too few input arguments")
		}
		if !t.IsVariadic() && len(in) > n {
			panic("reflectlite: Call with too many input arguments")
		}
	}
	for _, x := range in {
		if x.Kind() == Invalid {
			panic("reflectlite: " + op + " using zero Value argument")
		}
	}
	for i := 0; i < n; i++ {
		if xt, targ := in[i].Type(), t.In(i); !xt.AssignableTo(targ) {
			panic("reflectlite: " + op + " using " + xt.String() + " as type " + targ.String())
		}
	}
	if !isSlice && t.IsVariadic() {
		...
		for i := 0; i < m; i++ {
			...
			if xt := x.Type(); !xt.AssignableTo(elem) {
				panic("reflectlite: cannot use " + xt.String() + " as type " + elem.String() + " in " + op)
			}
			slice.Index(i).Set(x)
		}
		origIn := in
	...
	nin := len(in)
	if nin != t.NumIn() {
		panic("reflectlite.Value.Call: wrong argument count")
	}
	nout := t.NumOut()

	argsArray := js.Global.Get("Array").New(t.NumIn())
	for i, arg := range in {
		argsArray.SetIndex(i, unwrapJsObject(t.In(i), arg.assignTo("reflectlite.Value.Call", t.In(i).common(), nil).object()))
	}
